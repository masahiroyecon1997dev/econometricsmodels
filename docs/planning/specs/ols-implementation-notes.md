...
- **`summary()`・`to_frame()`/`conf_int()`のDataFrame版は実装しない**（`ols-api-design.md`5章の確定方針、「薄いラッパー」というスコープに照らして見送っている）。**`predict()`は例外的にスコープに含める**（`new_data=None`で学習データの予測値、指定時は新規データの予測値を返す1メソッドに統一。Issue #86、設計は`ols-api-design.md`7章参照。本ドキュメント作成時点では未実装）。
