...
## engineコードレビュー（Issue #29で実施済み。発見のみ、修正は別issue）

`engine/src/linear/ols.rs`全体をパフォーマンス・ロジックの両観点でレビューした。レビュー範囲はols.rsのみ（engine_pybind側は対象外、ユーザー確認済み）。着眼点はIssue #28のプロファイリング結果（HACのkスケーリング）と、issue本文が示した`hc_cov_params`と`hac_cov_params`の実装パターンの非対称性。実測はすべて`cargo test -p engine --release`のスクラッチベンチマーク（コミットしていない、使い捨てコード）で行った。

### 発見1（要修正・確認済み）: `hac_cov_params`の三重ループをfaer行列積に置き換えられる

issue本文の指摘通り、`Ŝ_l`（残差でスケールした行列`Xe`、`Xe[t,a] = e_t・x_t[a]`を使えば`Ŝ_l = Xe[l:,:]ᵀ @ Xe[:n-l,:]`という行列積）に書き換え可能。`l=0`項（`Ŝ₀`）も同様に`Xeᵀ @ Xe`という単一の行列積に置き換えられる（現状は`hac_cov_params`内で専用の二重ループになっている）。

実測（n=10,000固定、Issue #28のkスイープと同じk=2,5,10,20、ラグ数は自動計算式で約11）:

| k | 現状（三重ループ） | 素朴な行列積書き換え（faer既定の並列化） | 同、`faer::set_global_parallelism(Par::Seq)`適用後 |
|---|---|---|---|
| 2 | 1.02ms | 7.58ms（**0.13倍＝悪化**） | 0.21ms（**5.01倍**） |
| 5 | 3.71ms | 9.46ms（0.39倍＝悪化） | 0.43ms（8.33倍） |
| 10 | 12.52ms | 20.28ms（0.62倍＝悪化） | 0.78ms（17.12倍） |
| 20 | 49.39ms | 18.70ms（2.64倍） | 2.14ms（21.89倍） |

**重要な注意点**: ラグごとに`faer`の行列積を呼ぶ素朴な書き換えは、faer既定の並列化（グローバルにスレッドプールへディスパッチする）のオーバーヘッドが、k=2〜10程度の小さい行列積では計算本体より支配的になり、**現状の三重ループより遅くなる**（k=2で0.13倍）。`faer::set_global_parallelism(Par::Seq)`で逐次実行に切り替えると全k帯でオーバーヘッドが消え、5〜22倍高速化する。修正issueでは、この書き換えと同時に並列化設定（グローバル設定を変えるか、この計算だけスコープを切って`Par::Seq`にするか）をセットで扱う必要がある。単純に「三重ループを行列積に変えるだけ」だとリグレッションになりうる点を要注意事項として引き継ぐ。

正確性は`k=4, lags=5`のランダム値データで現状実装との相対誤差1e-6未満で確認済み。

**【Issue #48で対応済み】** 上記の通り`hac_cov_params`を`faer::linalg::matmul::matmul`ベースの実装に書き換え、`Par::Seq`をこの関数内だけにスコープを切って明示指定した（グローバルな並列化設定は変更していない）。書き換え後、`benchmark/compare_performance.py`のk軸スイープ（n=10,000固定、release build、repeats=7）で全k帯で明確な高速化を確認した（リグレッションなし）。

| k | 書き換え前（Issue #28実測、エンドツーエンド） | 書き換え後（Issue #48実測、エンドツーエンド） |
|---|---|---|
| 2 | 0.0104s | 0.0029s（約3.6倍） |
| 5 | 0.0128s | 0.0030s（約4.3倍） |
| 10 | 0.0427s | 0.0116s（約3.7倍） |
| 20 | 0.0914s | 0.0292s（約3.1倍） |

エンドツーエンド計測（Python API呼び出し込み）のため、`hac_cov_params`単体のマイクロベンチマーク（5〜22倍）より改善率は小さいが、Python/Arrow変換等のオーバーヘッドが相対的に増えるため想定通り。全k帯で改善しリグレッションが無いことを確認済み。既存のcargo test（HAC関連テスト含む全25件）・pytest（109件）・clippy・fmtもすべてグリーン。

### 発見2（探索したが不採用）: `xtx_inverse`をQR分解の`R`因子から導出する案

`fit()`は既に`col_piv_qr()`で`X`のQR分解を1回計算しているが、`xtx_inverse`は`(X'X)⁻¹`を求めるために`x.transpose() * x`で改めて`X'X`を計算し直している（QRの`R`因子を再利用していない）。`X'X = PᵀRᵀRP`（`P`は列ピボット）という関係を使えば、`R`（k×k）だけから`(X'X)⁻¹`を導出でき、`X'X`の明示的な計算（O(n·k²)）を避けられるのではと仮説を立てたが、**実測で否定された**。

n=1,000,000, k=5での実測:
- 現状（`x.transpose() * x` + Cholesky）: 25.1ms
- `col_piv_qr()`単体の所要時間: 103.2ms（既存の`X'X`計算より4倍遅い。faerのQR分解はピボット選択のため逐次的な依存が強く、`X'X`計算のような単純な行列積ほど並列化・SIMD化の恩恵を受けにくいと考えられる）
- QR再利用の限界費用（QR分解が`fit()`で既に払われている前提）: 21.7ms — 現状の25.1msと大差なし

結論: `fit()`が既に`col_piv_qr()`を呼んでいるという前提を差し引いても、QR由来の`R`から`(X'X)⁻¹`を導出する経路に明確な高速化余地はない。**修正issueは起票しない**。将来同じ発想が再検討されないよう記録として残す。

### 参考として確認したが対応不要と判断した点
