...
対応するCLAUDE.mdの方針: 8章（バージョニング・CI/CD）、5章（Git運用: `release/vX.Y.Z` → `dev` → `main`のPR経由、自動セルフマージはしない）
...
1. 現在のブランチ（`release/vX.Y.Z`で、バージョンバンプのコミットがある想定。ブランチ名が異なる・コミットが無ければユーザーに確認する）をpushし、`dev`へのPRを作成する。`main`/`dev`へは直pushせず、必ずPR経由にする。
