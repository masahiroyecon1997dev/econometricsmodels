...
### 26.【確認・対応不要】`test_iv_crosscheck.py`にGMMのRクロスチェックが無いのは`iv-api-design.md`5.3節で明記済みの意図的な例外規定
...
- **内容**: ユーザー指摘（2026-08-31、「benchmarkで指摘したかもしれ
  ...
  確認したところ、これは見落としではなく`iv-api-design.md`5.3節に
  明記された**意図的な設計判断（例外規定）**だった。`ivreg`パッケージ
  自体がGMM推定に対応していないというツール側の制約が根拠で、
  「R側で別パッケージ（`gmm`等）を新規導入するコストは掛けない」という
  判断も明記されている。パネルデータのHausman検定（`panel-api-design.md`
  5.3節）でも同種の前例があるとのことで、場当たり的な例外ではなく
  一貫した方針として扱われている。
- **Claudeの所感**: 対応不要と判断する。ドキュメント上も
  `test_iv_gmm_fixtures.py`のモジュールdocstring上も「GMMはivregが
  対応していないため対象外（5.3節、Rクロスチェック省略の例外規定）」と
  明記されており、実装漏れではなく承認済みの設計判断であることが
  一貫して記録されている。良好な状態だと考える。
- **気づいた経緯**: 2026-08-31、`tests/test_iv_crosscheck.py`解説時の
  ユーザー指摘、`iv-api-design.md`5.3節で確認。
- **状態**: 対応不要と判断（意図的な設計判断、`iv-api-design.md`5.3節に
  明記済み）
