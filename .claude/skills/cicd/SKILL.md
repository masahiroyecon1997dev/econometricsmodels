...
1. 対象が以下のどれかを確認する。
   ...
   - `benchmark_performance.yml`: `performance/compare_<method>.py`（＋共通ハーネス`performance/_perf_harness.py`）を手法ごとのmatrixジョブ（`method: [ols, wls, ...]`）で定期実行、job summaryへの結果出力。タグpush + `workflow_dispatch`トリガー。
