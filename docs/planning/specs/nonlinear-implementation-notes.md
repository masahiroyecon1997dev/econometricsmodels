...
**現状**: Logit/Probit/Tobitは実装完了。手法ごとの確定仕様は`docs/spec/logit-spec.md`・`probit-spec.md`・`tobit-spec.md`へ集約済み。本ノートには**nonlinear系統で3手法が共有する内部実装の設計判断**（エラー型`MleError`、`cov_type`共通行列演算、`run_solver`、標準化、限界効果の共通骨格等）のみを残す。
...
- L-BFGSは組み込みソルバーの`.with_tolerance_grad(tol)`（勾配のL2ノルムがこの値未満で収束と判定、`ArgminL2Norm`トレイト）をそのまま使う。線形探索は`MoreThuenteLineSearch`。**Issue #285以降、`run_solver`が渡す値は`tol`そのものではなく`tol * n_obs`**（下記「収束判定の`tol`」参照、`newton`は非該当）。
...
- **判定基準はmethodにより異なる（Issue #285で変更）**: `newton`は総和勾配に対する絶対閾値`‖∇ℓ(θ)‖ < tol`のまま（独自実装`FaerNewton`）。`bfgs`/`lbfgs`は観測数`n`で正規化した「観測あたり平均勾配」基準`‖∇ℓ(θ)‖ / n < tol`を使う（独自実装`FaerBfgs`／組み込み`with_tolerance_grad`、実装上は`run_solver`が`tol * n_obs`を実効的な絶対閾値として渡す形。`n_obs`は各モデルの`fit()`が観測数をそのまま渡す）。当初は3手法とも同じ絶対閾値基準だったが、`bfgs`/`lbfgs`は超1次収束のため`n`が大きいほど同じ絶対閾値を満たすのに必要な反復が増える一方`newton`は2次収束でこの影響をほぼ受けないという非対称性があり、statsmodels（scipy）が`n`で正規化してから最適化する設計に倣ってこの差を解消した（`newton`にも同じ正規化を適用する案は、`near_separation`等の`RTOL=1e-8`精度検証テスト28件を壊すため実測の上で不採用。詳細は`docs/spec/logit-spec.md`3.2節）。Issue #285でBFGSを自前実装に置き換えた際、副次的なコスト変化ベースの収束判定〈`|prev_cost-cost|<f64::EPSILON`〉も組み込みBFGSと同じ基準で明示的に踏襲している（これは正規化と無関係、`newton`/`bfgs`/`lbfgs`とも同じ）。
- **デフォルト値もmethodにより異なる**: `newton`は`tol=1e-6`（絶対閾値、変更なし）。`bfgs`/`lbfgs`は`tol=1e-8`（正規化基準）。共有の単一既定値は採れない——`newton`の既定を`1e-8`に締めると大標本で無視できない速度低下（実測: n=1,000,000で0.98s→3.15s）が生じる一方、`bfgs`/`lbfgs`の既定を`1e-6`に緩めると`n=500`の精度検証テストを壊すため。`engine_pybind`側の`LogitOptions`/`ProbitOptions`/`TobitOptions`は`tol: Option<f64>`（既定`None`）にし、`None`のとき`method`文字列を見てどちらの既定値を使うかコンストラクタ内で分岐する。当初（Logit実装時）確認した`tol=1e-6`の妥当性（統計モデルとの数値照合、境界ケースでの`1e-8`が必要になる知見）は`newton`にそのまま引き継がれる（結論・根拠は`docs/spec/logit-spec.md`「最適化・収束判定」参照）。
...
- **Options化**: `max_iter`と同じ扱いで`tol`をOptionsに追加する（`run_solver`関数の引数として実装済み。`n_obs: usize`引数も追加済み。engine_pybind側の配線はLogit/Probit/Tobitいずれも実装済み）。
...
`docs/spec/tobit-spec.md`へ集約した（打ち切り境界のバリデーション、`(β, log σ)`パラメータ化と`(β, σ)`空間への変換、`TobitScaling`（`y`スケーリング＋`x`平均センタリング）、`regularized_newton_step`、Wald検定（`llnull`不使用）、限界効果の3対象と`dydx_j = w(θ)·βⱼ`分解、`censoring_fit_check`、`(準)`完全分離が`σ→0`退化として現れるため`SeparationNormCheck::Disabled`、非識別データの`NoUncensoredObservations`検出、engine_pybind／python_package層の配線）。
