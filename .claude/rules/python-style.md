...
- **docstringは英語にする**（モジュール・クラス・関数・メソッド・プロパティの`"""docstring"""`全て）。`rust-style.md`「言語方針」が`engine_pybind`の`#[pyclass]`/`#[pyfunction]`のdocコメント（PyO3経由でPythonの`__doc__`になるもの）を英語にする方針と揃え、mkdocs（mkdocstrings）で同じページにレンダリングされるRust側・Python側のdocstringで言語が混在しないようにする（Issue #111で発覚した不整合の是正）。PyPI公開の独立パッケージであり、Pythonエコシステムの慣習（pandas/numpy/polars等）に合わせる、という`rust-style.md`と同じ理由。
