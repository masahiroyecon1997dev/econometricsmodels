...
- **`run_solver`内の判定位置**: 3手法共通の後処理（`extract_outcome`呼び出し後、`SolverOutput`構築前）に1箇所だけ実装。`converged && separation_suspected(&params)`（`l2_norm(params) > SEPARATION_PARAM_NORM_THRESHOLD`）のとき`converged`を`false`に取り消し、`raise_on_non_convergence=true`なら`SeparationSuspected`を即座に返す（`false`なら通常の非収束と同じく`converged=false`のまま後続処理を継続）。
- **理論的な限界（rust-reviewerの指摘、記録のみ・未対応）**: この方式が検出に使う量（標準化パラメータ空間でのL2ノルム`‖θ_std‖`）と、実際にアンダーフローを引き起こしている量（線形予測子`z_i = x_std_i·θ_std`の絶対値の最大値）は厳密には別物。`‖θ_std‖`が大きければ典型的には`|z_i|`も大きくなりやすいが、これは相関的な関係であり数学的に保証された関係ではない（例: 説明変数の分布が非対称・裾が重い、または特定の1列だけが分離に寄与し他の係数はほぼ0、というケースでは`‖θ_std‖`が閾値未満でも一部観測の`|z_i|`が極端に大きくなりうる）。`max_i |x_std_i·θ_std|`を直接見る方式の方が理論的に直接的だが、`run_solver`は各観測の`x`を保持していない（`params`のみ受け取る設計）ため実装コストが上がる。Probit/Tobitへの展開時に再検討の余地がある候補として記録する。
- **閾値のk依存性（rust-reviewerの指摘、記録のみ・未対応）**: `SEPARATION_PARAM_NORM_THRESHOLD=100.0`はn=200・k=3（切片含む）の単一DGP・単一seedでの実測にのみ基づく較正値。`‖θ_std‖`はL2ノルムのため、パラメータ数kが増えるほど各成分が中程度でも合計ノルムは大きくなりやすい。真に分離していないが単にkが大きい多変量モデルで誤検知するリスクは未検証。`‖θ_std‖/√k`のような正規化、または`max_j |θ_std_j|`（成分ごとの最大絶対値）を使う代替案を、他手法・多変量ケースへの適用時に検討する。
- **テスト**: 准分離データ（`beta1=100`の合成DGP）で、Newton/BFGS/LBFGSいずれも`SeparationSuspected`を返すことを確認する統合テストを追加。`raise_on_non_convergence=false`時に`converged=false`を返すことも別テストで確認。判定ロジック自体（`separation_suspected`関数）を`run_solver`から切り出し、閾値の直下・直上・ちょうど閾値の3ケースを直接検証する単体テストも追加した（実際の最適化トラジェクトリでは、`beta1`を振っても浮動小数点飽和により`y`の実現値が広い範囲で同一になり、閾値付近の境界データを意図的に作るのが難しいと判明したため。rust-reviewer指摘）。`beta1=20`（既存の`near_separation`シナリオと同じ設計思想の境界ケース）で3手法とも正常収束することを固定するリグレッションテストも追加（閾値を将来調整する際、既存の合格ケースを誤検知させないことを保証する）。既存135件（本Issue着手前）は全てパスしたまま。
- `cargo test -p engine`（141件）・`cargo test -p engine_pybind`（31件）・`pytest tests/api_tests`（365件、Python境界テスト追加後）、`cargo clippy --all-targets -- -D warnings`・`cargo fmt --check`・ruffいずれもパス。

### rust-reviewerレビューで対応した指摘

- **`gradient_norm`のリネーム**: 「パラメータのノルム」計算にも流用していたが、関数名が「勾配ノルム」を指すままだったため、中立的な`l2_norm`にリネームした。
- **判定ロジックの切り出しとテスト容易性**: `separation_suspected(params: &[f64]) -> bool`として`run_solver`から切り出し、閾値の境界値を実際の最適化トラジェクトリに頼らず直接テストできるようにした（上記「テスト」参照）。
- **既存`near_separation`シナリオでの3手法統一の誤検知なし確認**: 上記「テスト」参照。
- 上記2点の理論的限界の記録: 上記参照。
- **Python境界（`tests/api_tests/`）でのテスト追加、および`test_non_convergence_raises_computation_error_with_tiny_max_iter`のdocstring更新**: ユーザー確認の上で両方対応した。`test_logit.py`に`test_separation_suspected_raises_computation_error_for_near_separation_data`（`beta1=100`の合成DGP、Python標準の`random`モジュールで生成。engineのRust単体テストとは異なる乱数生成器だが、`beta1=100`は閾値100に対して十分大きなマージンがあるため生成器の違いに関わらず確実に`SeparationSuspected`を引き起こす）を追加し、`SeparationSuspected → ComputationError`の変換をAPIレベルで確認した。`test_non_convergence_raises_computation_error_with_tiny_max_iter`のdocstringも更新し、「完全分離データは今や`NonConvergence`ではなく`SeparationSuspected`という別のより具体的なエラーになるため、`NonConvergence`自体の確認には使えない」という現状に即した記述にした（Issue #138解消前の「アンダーフローで誤判定する既知の限界」という古い記述を置き換え）。
