...
- **`U_CLAMP`とNewton法（line searchなし）の相互作用（一部検証済み、Issue #316で追跡）**:
  `U_CLAMP`は一般化残差`λᵢ`のNaN化のみを防ぐ局所的な保護で、Hessianの重み
  `w=λᵢ(λᵢ+zᵢ)`が使う線形予測子`zᵢ`自体は無制限のまま。クランプ済み`λᵢ`と生の`zᵢ`を
  混在させるこの非対称性により、`|z|>U_CLAMP`かつ誤分類（`qᵢzᵢ`が大きく負）の観測では
  `w`が**負**になりうる（`λᵢ(λᵢ+zᵢ)>0`という大域凹性の前提が数値的に破れる）ことを
  Issue #284の調査時（2026-09-12）に数値実験で確認した。ただし現行のwarm start
  （3.2節、Issue #279）は最適化軌道を`|z|`の小さい領域に留めるため、標準的なデータでは
  この経路を実際には踏まない（`n=1,000,000, k=5`のbaseline DGPで実測: 全反復を通じて
  `|z|<8`）。より悪条件なwarm start・BFGS/L-BFGS経路（Newtonの停滞収束判定という
  セーフガードが無い）でなお発生しうるため、Issue #316で対応を検討中。
