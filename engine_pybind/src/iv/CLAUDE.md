...
- **`IVOptions`に`gmm_convergence: Option<f64>`（既定`None`）・`raise_on_non_convergence: bool`（既定`true`）を追加**（`GmmEstimator::fit`のシグネチャに対応、engine側で既に追加されていた引数が今回初めてPython側に配線された）。
