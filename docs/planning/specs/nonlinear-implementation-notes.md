...
- **Wald検定とクラスターロバストSEの構造的な相互作用（Issue #289で`ValidationError`へ統一）**: `fit()`はWald検定を`cov_type`の種類に関わらず常時実行する（`df_model==0`のときのみスキップ）ため、`cov_type=Cluster`のクラスターロバスト共分散`Ŝ=Σ_g S_gS_g'`が持つ構造的な制約がWald検定の`q×q`部分行列にも及ぶ。`Ŝ`はクラスター寄与スコアの総和がゼロ（MLEの一次条件`Σ_i s_i = 0`）になるため`rank(Ŝ) ≤ G - 1`であり、`G <= q`（`q = k - k_constant`）で`q×q`部分行列は**数学的に常に**特異（`G = q`ちょうども特異。Issue #220で「`G=q`はデータ配置次第で特異」と観測されたのは、`ensure_well_conditioned_symmetric_matrix`の相対閾値が丸め誤差込みの特異性を検出できるかがデータ依存だったため——特異性自体は常に成立）。`G`・`q`は入力だけから判定できるため、Issue #289で`fit()`冒頭のバリデーション（`validate_cluster_cov_type` → `validate_cluster_count_covers_slopes`、`CommonError::InsufficientClustersForInference`）へ前倒しし、OLS/WLS/Logit/Probit/Tobit/IV横断で`ValidationError`に統一した。Logit/Probitにとっては新規制約（従来は縮退した共分散から読んだSEを無警告で返すsilent-passだった、実質バグ）。旧・境界値回帰テスト（`fit_returns_computation_failed_when_wald_submatrix_is_singular_for_cluster_with_g_equals_q`）は`fit_returns_validation_error_when_cluster_count_equals_slopes`に置き換え。cov_type比較テスト等の「成功パス」は`G > q`（厳密不等号）を保つ。`G > q`でも傾き係数間の悪条件で`q×q`部分行列が数値的にほぼ特異になるケースは事前判定できないため`wald_chi2_test`内の`ensure_well_conditioned_symmetric_matrix`（`ComputationFailed`）がbackstopとして残る
