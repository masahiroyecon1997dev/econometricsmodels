...
- **改善余地**: engine の絶対性能は n=100,000 で 0.15〜0.16秒と実用上問題ないが、(a) quasi-Newton（lbfgs ~3.2x・bfgs 発散 #292）、(b) newton の大標本での Hessian 特異（#291）が engine 側の Tobit MLE の継続課題。cluster 経路は classical とほぼ同じ伸びで、現時点で特段の懸念はない。
