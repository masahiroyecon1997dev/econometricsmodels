...
`generate_ols_crosscheck_fixtures.py`と同じ（`run_lm_crosscheck.R`側で
...
使用例（リポジトリルートから）:
    python -m benchmark.linear.fixtures.generate_wls_crosscheck_fixtures
"""
    ...
    for scenario in NUMERIC_SCENARIOS:
        ...
        for cov_type in R_COV_TYPES:
            ...
            if cov_type == "hac":
                ...
                entry["r"] = run_lm_r(
                    csv_path, formula, cov_type, weight_col=WEIGHT_COLUMN_NAME
                )
...
def build_401ksubs_fixture(tmpdir: Path) -> dict:
    ...
    fixtures = {
        cov_type: {
            "r": run_lm_r(csv_path, formula, cov_type, weight_col="inv_inc")
        }
...
if __name__ == "__main__":
    run_fixture_cli(
        build_fixtures,
        BENCHMARKS_DIR / "wls_crosscheck.json",
        description=__doc__,
    )
