...
- α_iは推定済み係数から事後的に復元する（`α_i = ȳ_i - x̄_i'β̂`）。実装（Issue #184）では
  `within`変換時のグループ平均を保持する形は取らず、`FeInput`が保持する変換前の元の
  `y`/`x`/`entity`/`time`から都度再計算している（`quasi_demean_column`は拡張していない）。
- **2-wayはこの式をそのまま時点効果に拡張できない**（実装時に発見、ユーザー承認済み、
  2026-09-12）。`α_i`に定数`c`を足し`γ_t`から`c`を引いても同じ予測値になるため
  正規化の任意性があり、`α_i = ȳ_i. - x̄_i.'β̂`・`γ_t = ȳ_.t - x̄_.t'β̂`を単純に両方へ
  当てはめると大域平均`ȳ.. - x̄..'β̂`が二重計上されるバグになる。採用した規約:
  `time`の辞書順で最初の値`t_ref`を基準に`γ_{t_ref}=0`とし、`α_i`に大域的な水準を
  吸収させる（Stata `areg`・R `fixest`/`lfe`等と同型の「片方の基準カテゴリを0にする」
  慣行）。詳細な式・導出・`fixest::fixef()`との数値一致条件は
  `engine/src/panel/fe.rs`モジュールdoc「固定効果自体（α_i）の復元」を参照。
...
- **内部FE呼び出しの1-way/2-way選択（Issue #192実装時に判明した曖昧さ、ユーザー確認済み、
  ...
  **`ReOptions.time`が`Some`なら2-way FE（entity+time）を試みる**（`None`なら1-way FE）。
  ...
  既存フォールバック規定（「FE推定失敗時はハウスマン関連フィールドを`None`にする」）が
  そのまま適用される——「REは1-wayのみ、FEが2-wayのときのHausmanは常にNone」という代替案
  （REとFEの固定効果構造が食い違う場合にHausman検定の前提（H0下で両者とも一致推定量）が
  崩れるため）は不採用にせず、**2-way FEでの比較が成立する場合（バランスパネル等）は
  そのまま実施する**という判断（早すぎる悲観的None化を避ける）。
...
### 7.6 今後の拡張候補（2-way RE、v1スコープ外）

v1のREはentity方向のみ（7.1節・7.2節）。Issue #192実装時にハウスマン検定の内部FE呼び出しの
1-way/2-way判定（7.3節）を検討した際、RE自身を2-wayにスコープ拡張する案も出たが、
バランスパネルとアンバランスパネルで実装難易度が大きく異なるため、拡張の要否・方針は
別Issueに分離して検討する（ユーザー確認済み、2026-09-13）:

- Issue #327: バランスパネル限定の2-way RE（Wallace-Hussain法・Amemiya法等のANOVA型閉形式
  推定量が使える見込み）。
- Issue #328: アンバランスパネルの2-way RE（教科書レベルの閉形式が存在しない。第一候補は
  Wansbeek and Kapteyn (1989)の推定量）。
