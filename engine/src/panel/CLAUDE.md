...
- `fe.rs`: `FeInput`（入力データ型、Issue #175）。`y: Vec<f64>` / `x: Vec<Vec<f64>>` / `entity: Vec<String>` / `time: Option<Vec<String>>`等を保持する薄い入れ物で、`OlsInput`/`IvInput`と異なり`faer::Mat`は組み立てない（`quasi_demean_column`が列単位・`&[f64]`で動く設計のため、`fit()`実装（Issue #178）が生の列をそのまま渡せる。詳細は`fe.rs`モジュールdoc）。`from_columns`は次元検証（`y`↔各`x`列・`y`↔`entity`・`y`↔`time`）のみ行う。
  ...
  - `FeEffects`（`OneWay`/`TwoWay`）・`FeEstimator`（`OlsEstimator`への委譲、Issue #178、4.3節）も実装済み。`FeEstimator::fit`は`OlsEstimator::fit`を`CovType::Classical`固定・`OlsInput::from_columns`を`include_intercept=false`固定で呼ぶ（`cov_type`のentity単位cluster化デフォルト対応は#181で別途、Frisch-Waugh-Lovell定理によりwithin変換後のOLS点推定はFE推定量`β̂`と数学的に一致するため係数自体はこの暫定実装でも正しい）。パイプラインは「singleton検出（#179）→within変換（#176、2-wayはバランスパネル検証を内包）→自由度検証（#180）→分散ゼロ検出（#177）→`OlsEstimator::fit`への委譲→自由度調整後の統計量再計算（#180）」の順（`FeEstimator::fit`のdocコメント参照）。`OlsInput::from_columns`/`OlsEstimator::fit`が返す`LeastSquaresError`は`PanelError::WithinRegressionFailed { source }`に`.map_err`で明示的に包む（`common.rs`の方針、`#[from]`は使わない）。1-way/2-wayの切り替えは`FeEffects` enum（`bool`ではなく、`CovType`と同じ「小さな固定選択肢の公開enum」パターン、ユーザーとの相談で決定）を使う。将来`FeOptions`（Issue #186）が導入されたら、その一部として統合される想定の暫定パラメータ。
  - **自由度調整（Issue #180、6.3節）も実装済み**: `df_model = k + neffects`（1-wayは`neffects=n_entities`、2-wayは`neffects=n_entities+n_periods-1`。entity/timeダミー間の定数項ぶんの重複を`+1`補正）、`df_resid = n - df_model`。`n <= df_model`なら`PanelError::InsufficientDegreesOfFreedom`（この検証は`OlsEstimator::fit`自身の`n<=k`チェックより常に厳格——`neffects>=1`のため`df_model>k`が恒常的に成り立つ——なので、`FeEstimator::fit`経由でOLS側の`InsufficientObservations`が発生することは構造的にない）。`FeEstimator`は`OlsEstimator`が計算した標準誤差を**`sqrt(df_resid_ols/df_resid)`倍にスケールし直す**方式で自由度調整後の標準誤差を求める（`cov_params`の再構築が不要、`cov_type=Classical`前提でのみ成り立つ）。t値・p値・信頼区間は`crate::inference`の共有ヘルパー（OLS自身と同じロジック）でt分布（自由度`df_resid`）から計算し直す。調整済みR²は`estimator().r_squared()`（within R²、変換後yのuncentered TSSベース）ではなく**変換前の元のyの中心化TSSに基づくoverall R²**から`1 - (1-overall_R²)*(n-1)/df_resid`で計算する（残差はFWL定理により変換前後で同一のためSSRは`estimator.residuals()`をそのまま使える）。AIC/BICは`log_likelihood`自体は`SSR/n`のみに依存しdf非依存の式のため`OlsEstimator::log_likelihood()`をそのまま再利用し、ペナルティ項の乗数だけ`k`から`df_model`に差し替える。**F統計量はこの時点では未対応**（issue本文が「検定統計量（t検定）」と明示的に限定しているため、`estimator().f_statistic()`/`f_p_value()`は`df_resid_ols`ベースのまま）。
