...
- `benchmark/nonlinear/generate_nonlinear_datasets.py`: 2値選択モデル（Logit/Probit）専用の合成データ生成。`generate_binary_choice_dataset(scenario, link="logit"|"probit", ...)`（元はLogit専用だったが、Probit追加時に`link`引数で一般化した。シナリオ・X生成ロジックはOLSの`generate_linear_datasets.py`と別設計）。
...
- `benchmark/<系統>/run_statsmodels_benchmark_<系統>.py`（`linear`/`nonlinear`のみ、statsmodelsが主リファレンスの系統）: 主リファレンス。1回呼べば1ケース分の結果を返す汎用ツール。`linear`系統（`run_statsmodels_benchmark_linear.py`）では`--weight-col`指定でWLS（`smf.wls`）にも対応、`nonlinear`系統（`run_statsmodels_benchmark_nonlinear.py`）では`--model logit`/`--model probit`でLogit/Probitを切り替える。IV系統は主リファレンスがlinearmodelsのため`benchmark/iv/run_linearmodels_benchmark_iv.py`という別名（`run_gmm()`も同ファイル）。**両者とも元は`run_statsmodels_benchmark.py`という同名だったため、PYTHONPATH経由のimportで意図しない方が解決されるバグがあった。系統サフィックス付きに一意化して解消済み**（`refactoring-issue231-progress.md`参照）。
- `benchmark/<系統>/fixtures/generate_<手法名>_fixtures.py`: 対象手法の全シナリオ×全オプションを回し、`tests/fixtures/benchmarks/<手法名>.json`へ書き出す専用スクリプト。生成スクリプト（`benchmark/`側）と生成物（`tests/`側）を分けている（`testing-policy.md`「ベンチマーク値のフィクスチャ化」参照）。`linear/fixtures/generate_ols_fixtures.py`（statsmodels主リファレンス）・`generate_ols_crosscheck_fixtures.py`（Rクロスチェック）が実装例。フィクスチャ生成スクリプト自体は手法ごとに分ける（`run_statsmodels_benchmark_<系統>.py`のような汎用ツールとは異なる粒度、`generate_logit_fixtures.py`/`generate_probit_fixtures.py`のように別ファイル）。
...
2. `$ARGUMENTS`（手法名）に応じて、対象の全cov_type/オプションの組み合わせで`benchmark/<系統>/run_statsmodels_benchmark_<系統>.py`（IVは`run_linearmodels_benchmark_iv.py`）を実行する（固定済みCSVを読む）。
