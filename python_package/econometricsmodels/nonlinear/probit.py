"""Python wrapper for Probit (binary probit regression).

A thin wrapper around `econometricsmodels._lib.fit_probit` (the Rust
...
`ProbitOptions` is re-exported as-is from `_lib` (not redefined as a
separate class; same policy as `LogitOptions`, see
...
from .. import _lib
from .._lib import ProbitOptions

__all__ = ["Probit", "ProbitOptions", "ProbitResults"]


class Probit:
    """Binary probit regression estimator.
    ...
    Args:
        ...
        options: Estimation options. Defaults to `ProbitOptions()`
    ...
    def __init__(
        ...
        y: str,
        x: list[str],
        options: ProbitOptions | None = None,
    ) -> None:
        self._data = data
        self._y = y
        self._x = x
        self._options = options if options is not None else ProbitOptions()

    def fit(self) -> ProbitResults:
        """Estimate the Probit model by maximum likelihood.
        ...
        """
        raw = _lib.fit_probit(self._data, self._y, self._x, self._options)
        return ProbitResults(raw)
...
class ProbitResults:
    ...
    Args:
        raw: The estimation result object returned by `_lib.fit_probit`
            (`_lib.ProbitResult`).
    ...
    def __init__(self, raw: _lib.ProbitResult) -> None:
        self._raw = raw
    ...
    def lr_statistic(self) -> float:
        ...
        Probit analogue of OLS's F-statistic)."""
