...
- **同じ「`tol`が`n`でスケールしない」性質は、大標本での`bfgs`/`lbfgs`の実行時間にも直接影響する
  （Issue #285調査、2026-09-12）**: `newton`は2次収束のため、収束点に到達した後さらに勾配ノルムを
  数桁分小さくするのに要する追加反復はごく僅かで済む（`n`が大きく勾配の絶対スケールが大きい場合
  でも実質無償で吸収できる）。一方`bfgs`/`lbfgs`は超1次収束（2次収束ではない）のため、同じ絶対
  勾配閾値`tol`を満たすのに`newton`よりずっと多い反復・関数評価を要し、しかもその必要反復数は
  `n`が大きいほど増える（実測: `n`=100,000→1,000,000で`bfgs`の反復回数が16→17〜22、大幅な追加の
  関数評価を伴う）。これは実装（line search・逆Hessian近似の更新等）の不備ではなく、`tol`の意味論
  そのもの（生の総和勾配に対する絶対閾値）に起因する。statsmodels（scipy）は最適化前に対数尤度・
  スコア・Hessianを観測数`n`で割ってから最適化しており（`statsmodels.base.model.LikelihoodModel.fit`
  の`f=-loglike/nobs`等）、収束判定が実質的に`n`非依存なスケールになっているため、この影響を受けない。
  - **実測（`generate_binary_choice_dataset("baseline", link="logit", n=1_000_000, k=5, seed=42)`、
    `method="bfgs"`）**: 既定`tol=1e-6`だと反復回数17・実行時間約15秒。`tol`を`n`に比例させて
    `tol=1`（≒観測あたり平均勾配の閾値`1e-6`相当）にすると反復回数6・実行時間0.77秒（`newton`と
    同オーダー）に短縮され、係数の相対差は`tol=1e-6`時との比較で`5.4e-10`と実務上無視できる。
    `n`=100,000・300,000でも同様の傾向（相対差は`1e-6`〜`1e-9`程度）を確認済み。
  - **小標本では同じスケーリングを単純適用すると精度検証テストを壊しうる**: `tests/fixtures/
    benchmarks/logit.json`の`baseline`シナリオは`n=500`で生成されており（`.claude/rules/
    testing-policy.md`の基本方針`相対誤差1e-8`と厳密比較する）、`tol`を`n`倍する単純なスケーリング
    をこの`n`で試すと係数の相対差が`1.2e-6`〜`2.9e-6`程度に達し、既存の`RTOL=1e-8`比較を壊しうる
    ことを実測で確認済み。そのため**既定値・実装は変更していない**（ユーザー確認済み）。
  - **運用上の推奨**: 大標本（目安として`n`が数十万〜）で`bfgs`/`lbfgs`を使い、実行時間が問題になる
    場合は、`tol`を観測数`n`にほぼ比例させて指定する（例: `tol=n*1e-6`）ことで、係数の実務上の精度を
    ほとんど落とさずに`newton`と同程度の速度まで改善できる。既定の`newton`を使う場合はこの調整は
    不要（`newton`は`tol`の`n`依存性の影響をほとんど受けない）。
