...
3. **グループ/クラスター単位の分散推定（クラスターロバストSE等）を持つ手法**: 均等サイズの疑似グループだけでなく、不均衡なグループサイズ（例: `[2, 3, 5, 10, 30, 50]`のように偏らせる）、グループ数が境界値に近いケース（最小グループ数ちょうど、その外側の失敗パス）、実データでのグループ列も検証する。均等サイズの疑似グループのみのテストは、実務でバグが出やすい分布の偏りを見逃す。
   - **境界値ケースでは「クラスタ数G」と「傾き係数の数q」の関係に注意する**（OLSの実装過程で判明）。クラスターロバスト共分散行列はG個のランク1行列の和のため`rank ≤ G`であり、ロバストWald検定（F統計量）が使う`q×q`の部分行列はG<qのとき構造的に特異になりうる。係数・標準誤差自体はGが2以上であれば計算できるが、F検定を含む`fit()`全体がG<qで失敗しうる。「クラスタ数境界の成功パス」を作る際はqをG以下に保つ（詳細は`docs/planning/specs/ols-implementation-notes.md`「クラスター標準誤差」参照）。この関係はクラスターロバストSEを持つ他の手法（WLS/IV等）にも同様に当てはまるため、実装時に確認すること。
