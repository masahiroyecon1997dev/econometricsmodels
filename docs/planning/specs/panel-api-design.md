...
FE/RE固有の論点・実装仕様は、両手法の実装完了に伴い[`fe-spec.md`](../../spec/fe-spec.md)・
[`re-spec.md`](../../spec/re-spec.md)へ集約済み（本ドキュメントには残さない）。
...
- **RE: ハウスマン検定は`fit()`内で自動計算**し、`REResult`に`hausman_statistic` /
  ...
  - REとFEの内部設計共有範囲（同じ推定ルーチンを呼ぶか等）は4章、およびRE固有の詳細は
    [`re-spec.md`](../../spec/re-spec.md)を参照。
...
- **2-way clustering（entity+time同時）はv1スコープ外**。2-way FEのスコープ（2-way FEでも
  クラスターのデフォルトはentity単位のまま維持する。詳細は[`fe-spec.md`](../../spec/fe-spec.md)
  参照）と合わせて別途検討する。
...
この委譲パターンが実際にうまくいくかは、within変換の実装方法
（[`fe-spec.md`](../../spec/fe-spec.md)3.1節）次第のため、
