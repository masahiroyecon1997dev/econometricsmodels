...
- `fe.rs`: `FeInput`（入力データ型、Issue #175）。`y: Vec<f64>` / `x: Vec<Vec<f64>>` / `entity: Vec<String>` / `time: Option<Vec<String>>`等を保持する薄い入れ物で、`OlsInput`/`IvInput`と異なり`faer::Mat`は組み立てない（`quasi_demean_column`が列単位・`&[f64]`で動く設計のため、`fit()`実装（Issue #178）が生の列をそのまま渡せる。詳細は`fe.rs`モジュールdoc）。`from_columns`は次元検証（`y`↔各`x`列・`y`↔`entity`・`y`↔`time`）のみ行い、自由度調整（#180）は後続issueで追加する。
  ...
  - `FeEffects`（`OneWay`/`TwoWay`）・`FeEstimator`（`OlsEstimator`への委譲、Issue #178、4.3節）も実装済み。**この時点のスコープは係数推定（β̂）のみ**——自由度調整（`n - n_entities - k`、#180）・`cov_type`のentity単位cluster化デフォルト（#181）・パネル固有R²（#183）は明示的に別issue扱い（4.3節がWLSの教訓を踏まえて要求する区別）で、`FeEstimator::fit`は`OlsEstimator::fit`を`CovType::Classical`固定・`OlsInput::from_columns`を`include_intercept=false`固定で呼ぶ（Frisch-Waugh-Lovell定理によりwithin変換後のOLS点推定はFE推定量`β̂`と数学的に一致するため、係数自体はこの暫定実装でも正しい。**`estimator().std_errors()`等は正しくない前提**、`FeEstimator::estimator()`のdocコメント参照）。パイプラインは「singleton検出（#179）→within変換（#176、2-wayはバランスパネル検証を内包）→分散ゼロ検出（#177）→`OlsEstimator::fit`への委譲」の順（`FeEstimator::fit`のdocコメント参照）。`OlsInput::from_columns`/`OlsEstimator::fit`が返す`LeastSquaresError`は`PanelError::WithinRegressionFailed { source }`に`.map_err`で明示的に包む（`common.rs`の方針、`#[from]`は使わない）。1-way/2-wayの切り替えは`FeEffects` enum（`bool`ではなく、`CovType`と同じ「小さな固定選択肢の公開enum」パターン、ユーザーとの相談で決定）を使う。将来`FeOptions`（Issue #186）が導入されたら、その一部として統合される想定の暫定パラメータ。
